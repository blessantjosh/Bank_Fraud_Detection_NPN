...
**Investigator surface: Argus** (`dashboard/`), a FastAPI + static-frontend console, now reading this pipeline's artifacts: `ensemble_percentile_average` for the score, Phase 13's cutoffs for the tiers, and the precomputed Isolation Forest and Autoencoder SHAP rows shown side by side. Nothing on it is hand-typed from a report — every number is read from an artifact at startup, so a stale artifact produces a visibly stale dashboard rather than a silently wrong one. Its What-if Simulator required a **redesign rather than a repoint**: free-form new-transaction scoring is not honestly possible on a feature set built from population statistics, so it was rebuilt as an *Account Scenario Simulator* anchored to a real account's true historical frequency values (Phase 15 §7.3).

**The one thing Argus produces that nothing else in this project does: investigator decisions.** `queue_state.json` is the only mechanism here capable of generating labelled data, and every limitation in this report traces back to not having any.
...
1. **Capture investigator decisions from day one.** Argus's `queue_state.json` is the only label-generating mechanism in this project. A year of it makes a supervised model, a real precision/recall number, and a genuine cost-optimised threshold possible. **Everything in §13's list of limitations either dissolves or shrinks once labels exist.**

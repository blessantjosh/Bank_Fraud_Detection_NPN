...
| 8 | Model development (9 models) | `research/06_model_development.md` |
...
Nine unsupervised anomaly-detection models — 8 classical detectors plus a Hybrid Ensemble majority vote — were built, tuned, evaluated, explained, ensembled and thresholded against an unlabelled retail banking transaction dataset, and a production architecture and monitoring framework were designed on top of the result.

*A note on scope, stated up front rather than left to be discovered: this pipeline originally also built three deep-learning models — a plain Autoencoder, a Variational Autoencoder and an LSTM Autoencoder — as Models 9–11, with the Hybrid Ensemble (then Model 12) voting Isolation Forest + LOF + Autoencoder. All three deep-learning models were removed from this pipeline by an explicit, later project decision (they added training cost and a `torch` dependency without demonstrably outperforming the classical detectors on this small, unlabelled dataset), and the Hybrid Ensemble's third vote was switched to GMM. Every number, table and finding below reflects the resulting 9-model pipeline, regenerated end-to-end after the removal — not the original 12-model design.*

**What the system is.** A 46-feature engineering pipeline with verified leakage safety, feeding 6 to 8 independent anomaly detectors whose scores are combined by percentile aggregation into a single score in (0,1), thresholded into two human-review tiers, and explained to an investigator through Isolation Forest's SHAP attributions.

**The recommendation.** Percentile aggregation over the detectors that can score out-of-sample, with Isolation Forest SHAP attributions shown per alert, feeding a two-tier review queue at the 99th percentile (26 transactions, 1.04%) and the 95th percentile (126 transactions, 5.02%). **No automatic blocking.**

**The single most important technical finding.** The 9 detectors do not agree with each other nearly as much as their near-identical ~5% flagged rates would suggest, and one model in particular — GMM, the detector chosen as the Hybrid Ensemble's third vote alongside Isolation Forest and LOF — is the clearest outlier in the set. Its internal-validity Silhouette (0.319) sits far below every other model (next-lowest 0.565, highest 0.672), and it is the *only* model whose flagged set correlates negatively with another model's (Spearman ρ = **−0.052** against One-Class SVM, the sole negative pair among all 36 model pairs) and it agrees with DBSCAN at essentially chance level (ρ = 0.010). This is not noise; GMM's anomaly definition (low likelihood under a fitted Gaussian mixture) is genuinely a different question from every distance- or density-based detector's, and both the Silhouette gap and the correlation pattern say so independently. That divergence is exactly why the Hybrid Ensemble's majority-vote design (≥2 of 3 detectors must agree) matters: GMM alone would flag a materially different population than the consensus, and a single-model system built only on GMM would be flagging transactions none of the other 8 detectors find unusual.
...
**The load-bearing feature is `Amount_ZScore_Account`** — `(amount − account's prior expanding mean) / (prior expanding std)` — the most direct available answer to "how unusual is this for *this* customer." Its denominator is floored at **5% of the dataset-wide `TransactionAmount` standard deviation ($291.95 → $14.60)** rather than at an arbitrary epsilon. Phase 6 records why: an earlier build used a near-zero epsilon, and accounts with 2–3 near-identical prior transactions drove the feature into the hundreds of millions, swamping every other feature's contribution to any distance- or reconstruction-based model in the pipeline (this was originally diagnosed via the now-removed Autoencoder's loss, but the underlying numerical-stability problem is model-agnostic and the fix benefits every remaining model equally). Post-fix the feature ranges −92.60 to 102.80 — large, but explicable from genuine account-level variability.
